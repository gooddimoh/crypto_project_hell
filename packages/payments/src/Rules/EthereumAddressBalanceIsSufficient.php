...
class EthereumAddressBalanceIsSufficient implements Rule
{
    ...
    public function passes($attribute, $value)
    {
        ...
        $contractAddress = config('payments.ethereum.deposit_contract');

        // if ERC20 token contract address is specified
        if ($contractAddress) {
            $this->paymentService->fetchTokenBalance($this->address, $contractAddress);
            $decimals = config('payments.ethereum.deposit_contract_decimals');
        } else {
